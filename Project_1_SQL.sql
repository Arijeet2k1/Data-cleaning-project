...
-- DATA WRANGLING
	-- Summary of Overall data
		-- This data is all about smartphones. It shows detailed informartion, specificcations 
        -- and prices of each model of smartphones and this is an uncleaned data
...
-- Replacing all 0 in rating column with NULL
...
UPDATE smartphones
SET RAM_IN_GB = 3 WHERE `Index` = '763';
...
-- DELETING Index NUMBER 156 AND 763 BECAUSE THE DATA IS WRONG --
DELETE FROM smartphones 
WHERE `Index` = 156 OR `Index` = '763';
...
-- CONCLUDED THIS DATA AFTER SEARCHING MANUALLY
UPDATE smartphones
...
select * from smartphones where `BATTERY_CAPACITY_(mAh)` NOT like '____' AND `BATTERY_CAPACITY_(mAh)` NOT like '_____';
...
-- FIXING REMAINING ISSUES AFTER CHECKING ALL THE COLUMNS IN EXCEL

-- IN 2 ROWS THE PROCESSOR NAME WAS NOT GIVEN NOW THOSE 2 WORS ARE FIXED
SET SQL_SAFE_UPDATES = 0;
UPDATE SMARTPHONES 
SET Processor_name = 'Unisoc' 
WHERE Processor_name = 'SC9863A';

-- NOW EXTRACTING ONLY PROCESSOR NAME EXCLUDING THEIR GENERATIONS
UPDATE SMARTPHONES
SET Processor_name = SUBSTRING_INDEX(Processor_name,' ',1);

SELECT Processor_name FROM SMARTPHONES 
GROUP BY Processor_name;

-- ANALYZING ALL THE ROWS WHERE VALUEAS ARE 'NOT' AND CAN BE FILLED WITH OTHER VALUES
SELECT * FROM SMARTPHONES 
WHERE Processor_name = 'Not';

-- FOUND SUCH ROWS AND NOW FIXING THEM
UPDATE SMARTPHONES
SET Processor_name = 'Google Tensor'
WHERE processor LIKE '%Google%Tensor%';

UPDATE SMARTPHONES
SET Processor_name = 'Snapdragon'
WHERE `Index` = 492;

-- MARKING REMAINING ROWS AS NULL
UPDATE SMARTPHONES
SET Processor_name = NULL
WHERE Processor_name = 'Not';

-- FIXING REMAINING ISSUES IN FAST CHARGING COLUMN

-- SEPERATE COLUMN FOR FAST CHARGING SUPPORT
ALTER TABLE SMARTPHONES
ADD COLUMN FAST_CHARGING VARCHAR(50) 
AFTER `BATTERY_CAPACITY_(mAh)`;


UPDATE SMARTPHONES
SET FAST_CHARGING = 'Supported'
WHERE FAST_CHARGING_SPEED != 0;

UPDATE SMARTPHONES
SET FAST_CHARGING = 'Supported'
WHERE battery LIKE '%Fast%Charging%' AND FAST_CHARGING_SPEED = 0;

UPDATE SMARTPHONES
SET FAST_CHARGING = 'Info Not Available'
WHERE FAST_CHARGING IS NULL;

UPDATE SMARTPHONES 
SET FAST_CHARGING_SPEED = 22
WHERE battery LIKE '%22.5%W%Fast%Charging%' AND FAST_CHARGING_SPEED = 0;

UPDATE SMARTPHONES 
SET FAST_CHARGING_SPEED = 22
WHERE battery LIKE '%68.2%W%Fast%Charging%' AND FAST_CHARGING_SPEED = 0;

UPDATE SMARTPHONES 
SET FAST_CHARGING_SPEED = NULL
WHERE FAST_CHARGING_SPEED = 0;

-- CHANGING ALL THE 0 TO NULL

UPDATE SMARTPHONES
SET REFRESH_RATE_IN_HZ = NULL
WHERE REFRESH_RATE_IN_HZ = 0;

-- ADDING NEW COLUMN BY EXTRACTING DATA FROM REAR_CAMERA COLUMN TO MAKE IT A CATEGORICAL COLUMN

ALTER TABLE SMARTPHONES 
ADD COLUMN NO_OF_REAR_CAMS INT AFTER REAR_CAMERA;

SET SQL_SAFE_UPDATES=0;
UPDATE SMARTPHONES 
SET NO_OF_REAR_CAMS = 3
WHERE REAR_CAMERA LIKE '%MP%MP%MP  ';

UPDATE SMARTPHONES 
SET NO_OF_REAR_CAMS = 2 
WHERE REAR_CAMERA LIKE '%MP%MP  ' AND REAR_CAMERA NOT LIKE '%MP%MP%MP  ';

UPDATE SMARTPHONES 
SET NO_OF_REAR_CAMS = 1
WHERE REAR_CAMERA NOT LIKE '%MP%MP%MP  ' AND REAR_CAMERA NOT LIKE '%MP%MP  ';

-- ANALYZED REMAINING FAULTS AND FOUND THAT 4 ROWS HAVE INCORRECT INFO
-- FIXING THOSE ISSUES 
UPDATE SMARTPHONES
SET NO_OF_REAR_CAMS = 3
WHERE `INDEX` IN (449,629,854);

UPDATE SMARTPHONES
SET NO_OF_REAR_CAMS = 2
WHERE `INDEX` = 1017;

-- ADDING NEW COLUMN BY EXTRACTING DATA FROM FRONT_CAMERA COLUMN TO MAKE IT A CATEGORICAL COLUMN
SELECT * FROM SMARTPHONES WHERE FRONT_CAMERA LIKE '%MP%MP%';

ALTER TABLE SMARTPHONES 
ADD COLUMN NO_OF_FRONT_CAMS INT AFTER FRONT_CAMERA;

UPDATE SMARTPHONES
SET NO_OF_FRONT_CAMS = 1 
WHERE FRONT_CAMERA NOT LIKE '%MP%MP%';

UPDATE SMARTPHONES
SET NO_OF_FRONT_CAMS = 2
WHERE FRONT_CAMERA LIKE '%MP%MP%';

-- ADDING NEW COLUMN BY EXTRACTING DATA FROM MEMORY_CARD COLUMN TO MAKE IT A CATEGORICAL COLUMN

ALTER TABLE SMARTPHONES
ADD COLUMN MEMORY_SUPPORT VARCHAR(50) AFTER MEMORY_CARD;

UPDATE SMARTPHONES
SET MEMORY_SUPPORT = 'Supported'
WHERE MEMORY_CARD != 'Memory Card Not Supported' AND MEMORY_CARD != 'NOT AVAILABLE';

UPDATE SMARTPHONES
SET MEMORY_SUPPORT = 'Not Supported'
WHERE MEMORY_CARD = 'Memory Card Not Supported';

UPDATE SMARTPHONES
SET MEMORY_SUPPORT = 'Info Not Available'
WHERE MEMORY_CARD = 'NOT AVAILABLE';

-- ADDING NEW COLUMN MEMORY_STORAGE_IN_GB BY EXTRACTING DATA FROM MEMORY_CARD COLUMN 
ALTER TABLE SMARTPHONES
ADD COLUMN MEMORY_STORAGE_IN_GB VARCHAR(50) AFTER MEMORY_SUPPORT;

UPDATE SMARTPHONES T1
SET MEMORY_STORAGE_IN_GB = (SELECT VAL FROM (
SELECT `INDEX`,
	CASE 
		WHEN MEMORY_CARD LIKE '%upto%' THEN REPLACE(TRIM(SUBSTRING_INDEX(MEMORY_CARD,'upto',-1)),'GB','')
		ELSE NULL
	END VAL
FROM (SELECT * FROM SMARTPHONES) T) T2
WHERE T1.`INDEX` = T2.`INDEX`);

-- NOW COVERTING DATA FROM TB TO MB
UPDATE SMARTPHONES 
SET MEMORY_STORAGE_IN_GB = 1024
WHERE MEMORY_STORAGE_IN_GB = '1TB';

UPDATE SMARTPHONES 
SET MEMORY_STORAGE_IN_GB = 2048
WHERE MEMORY_STORAGE_IN_GB = '2TB';

-- CHANGING THE DATATYPE TO INT
ALTER TABLE SMARTPHONES
MODIFY MEMORY_STORAGE_IN_GB INT;

SELECT * FROM SMARTPHONES;

-- CHANGING OPERATING SYSTEM COLUMN INTO CATEGORICAL COLUMN B/W ANDROID, IOS, OTHERS
SELECT * FROM SMARTPHONES WHERE OPERATING_SYSTEM LIKE '%Android%';
SELECT * FROM SMARTPHONES WHERE OPERATING_SYSTEM LIKE '%iOS%';
SELECT * FROM SMARTPHONES WHERE OPERATING_SYSTEM NOT LIKE '%Android%' AND OPERATING_SYSTEM NOT LIKE '%iOS%';

UPDATE SMARTPHONES 
SET OPERATING_SYSTEM = 'Android'
WHERE OPERATING_SYSTEM LIKE '%Android%';
...
UPDATE SMARTPHONES 
SET OPERATING_SYSTEM = 'Others'
WHERE OPERATING_SYSTEM NOT LIKE '%Android%' AND OPERATING_SYSTEM NOT LIKE '%iOS%';

-- collecting the data of primary camera from REAR_CAMERA column
select PRIMARY_FRONT_CAM from smartphones
group by PRIMARY_FRONT_CAM;

ALTER TABLE SMARTPHONES
ADD COLUMN PRIMARY_RARE_CAM VARCHAR(50) AFTER REAR_CAMERA;

SELECT * FROM SMARTPHONES WHERE PRIMARY_FRONT_CAM LIKE '%Main%';


SET SQL_SAFE_UPDATES = 0;
UPDATE SMARTPHONES
SET PRIMARY_RARE_CAM = SUBSTRING_INDEX(REAR_CAMERA,'MP',1);

ALTER TABLE SMARTPHONES
MODIFY PRIMARY_RARE_CAM FLOAT;


-- collecting the data of primary camera from FRONT_CAMERA column

ALTER TABLE SMARTPHONES
ADD COLUMN PRIMARY_FRONT_CAM VARCHAR(50) AFTER FRONT_CAMERA;

UPDATE SMARTPHONES
SET PRIMARY_FRONT_CAM = SUBSTRING_INDEX(FRONT_CAMERA,'MP',1);

ALTER TABLE SMARTPHONES
MODIFY PRIMARY_FRONT_CAM FLOAT;

-- FIXING 1 QUALITY ISSUE IN THE DATA
UPDATE SMARTPHONES
SET PRIMARY_FRONT_CAM = 20 WHERE PRIMARY_FRONT_CAM LIKE '%Main%';

SELECT PRIMARY_RARE_CAM,NO_OF_REAR_CAMS,PRIMARY_FRONT_CAM,NO_OF_FRONT_CAMS FROM SMARTPHONES;


-- ADDING ALL THE CLEANED DATA TO A NEW TABLE 
CREATE TABLE SMARTPHONES_CLEANED AS
(SELECT `Index`, brand_name, model, price, rating, `5g_Available`, 
Processor_name,RAM_IN_GB, ROM_IN_GB,
 `BATTERY_CAPACITY_(mAh)`,FAST_CHARGING,
 FAST_CHARGING_SPEED,DISPLAY_IN_INCHES, 
 PIXLES_IN_WIDTH, PIXLES_IN_HEIGHT, 
 REFRESH_RATE_IN_HZ,PRIMARY_RARE_CAM,NO_OF_REAR_CAMS, PRIMARY_FRONT_CAM,
 NO_OF_FRONT_CAMS,MEMORY_SUPPORT,
 MEMORY_STORAGE_IN_GB,OPERATING_SYSTEM FROM smartphones);